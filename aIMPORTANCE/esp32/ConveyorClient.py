...
class ConveyorBelt:
    def __init__(self, cfg: ConveyorConfig):
        ...
        # concurrency flag: nếu True thì hệ thống đang thực hiện 1 action
        self.busy = False
        ...
        for t in cfg.mqtt_topics_cmd:
            self.mqtt_cfg['topics'].append((t, self.on_mqtt_msg))
    ...
    def led_state(self, state):
        # giả sử neopix & hex_to_rgb tồn tại
        try:
            neopix.show(0, hex_to_rgb(COLORS.get(state, '#000000')))
        except Exception:
            # nếu không có neopix, vẫn in log
            pass
    ...
    async def init_servo_motor(self):
        ...
        async def perform_action(self, servo=None, action_id=None, duration_ms=None):
            ...
            try:
                ...
                # 1️⃣ Đưa servo đến vị trí làm việc
                if servo:
                    try:
                    ...
                    await asleep_ms(200)  # chờ ổn định

                # 2️⃣ Chạy motor trong khoảng thời gian định sẵn
                print(f"[{utime.ticks_ms()} ms] Motor running...")
                self.motor.run(self.cfg.speed_motor)
                await asleep_ms(duration)

                # 3️⃣ Dừng motor, chờ ổn định trước khi trả servo
                print(f"[{utime.ticks_ms()} ms] Motor stopping...")
                self.motor.run(0)
                await asleep_ms(300)  # đệm nhỏ để dừng hoàn toàn

                # 4️⃣ Trả servo về vị trí mặc định MỘT LẦN duy nhất, mượt
                if servo:
                    print(f"[{utime.ticks_ms()} ms] Servo returning to {self.cfg.default_servo}")
                    # di chuyển chậm hơn khi trả về
                    smooth_speed = max(20, int(self.cfg.speed_servo * 0.5))
                    await servo.run_angle(self.cfg.default_servo, smooth_speed)
                    await asleep_ms(200)  # giữ ổn định sau khi trả
    ...
    async def idle_action(self, duration_ms=None):
        # Nếu đang bận thì bỏ qua idle action
        if self.busy:
            # không log quá dày, chỉ in thỉnh thoảng nếu cần
            return
    ...
    async def on_mqtt_msg(self, topic, msg):
        # topic may be e.g. 'V1' (because mqtt_as delivers topic part)
        ...
        # If system busy, respond BUSY (don't queue)
        if self.busy:
        ...
        if action == 1:
            ...
            # action 3 has no servo, just motor-based action
    ...
    async def led_blink_task(self, interval_ms=500):
        while True:
            # Nếu đang bận thì đợi, không chớp LED
            if not self.busy:
                self.led_state('RUN')
                ...
                # ngắn ngủi chờ để kiểm tra lại tình trạng busy
                await asleep_ms(100)
    ...
    async def handle_boot(self, pressed):
        if pressed:
            # nếu đang bận thì bỏ qua hoặc bạn có thể queue
            if self.busy:
            ...
            # trả servo về 0 ngay (bỏ qua busy)
            self.motor.run(0)
            try:
                await self.servo1.run_angle(self.cfg.default_servo, self.cfg.speed_servo)
            except Exception:
                pass
            try:
                await self.servo2.run_angle(self.cfg.default_servo, self.cfg.speed_servo)
            except Exception:
                pass
            self.led_state('IDLE')
    ...
    async def setup(self):
        ...
        # start blink task (it will not override when busy)
    ...
    async def run(self):
        ...
        while True:
            ...
            if utime.ticks_diff(now, last_idle) > self.cfg.idle_interval_ms:
                # only run idle_action when not busy
                if not self.busy:
